...
export const chatWithNana = async (
  ...
  if (chatModelId.includes('doubao') || chatModelId.includes('glm')) {
    ...
    // 路由到 Gemini
    return await sendGeminiChatMessage(history, newMessage, isImagePaused, chatModelId, imageModelId, contextData, layer1Intent, coachFeedback, customPersona);
  }
};
...
export const nanaSelfAct = async (
  ...
  if (chatModelId.includes('doubao') || chatModelId.includes('glm')) {
    ...
    if (content) {
        ...
        const tts = await generateExternalTTS(volcResult.text, "playful and cute");
        if (tts.audio) {
            audio = tts.audio;
            duration = tts.duration;
        }
