...
// 系统事件类型：新增 POST_MOMENT
export type SystemEventType = 'WAKE_UP' | 'SLEEP' | 'COMMUTE' | 'WORK_IDLE' | 'LUNCH' | 'BROWSING' | 'NONE' | 'REFLECTION' | 'POST_MOMENT';
...
export interface ScheduleRule {
  ...
  description: string;
  location: string; 
  outfitCategory: 'HOME' | 'WORK' | 'GYM' | 'NIGHT'; 
  goal: string; 
  isOneTime?: boolean; 
  forceImage?: boolean; 
}
...
export interface AIStatus {
  ...
  dailyMomentCount: number; // New: 每日朋友圈计数
...
export interface AIContextType {
  ...
  addMoment: (moment: Moment) => void; // New
